...
class LeadScorer:
    ...
    def _score_urgency(self, lead_data: Dict[str, Any]) -> int:
        """Score urgency 0-100 based on message and fields."""
        text = self._extract_text(lead_data).lower()

        # Check for high urgency signals
        if any(kw in text for kw in self.URGENCY_HIGH):
            return 100
        if any(kw in text for kw in self.URGENCY_MED):
            return 60
        if any(kw in text for kw in self.URGENCY_LOW):
            return 30

        # Default based on form completeness
        required_fields = ['email', 'phone', 'company_name']
        filled = sum(1 for f in required_fields if lead_data.get(f))
        return 20 + (filled * 15)  # 20-65 based on completeness
